...
if len(sys.argv) != 2:
    print(
        f"""Usage: python {sys.argv[0]} <release filename-version>

eg. python {sys.argv[0]} my_cool_game-1.0"""
    )
    sys.exit()

base = sys.argv[1]
zipname = f"dist/{base}.zip"
...
# core files
for name in "README.txt run_game.py requirements.txt euclid.py".split():
...
def add_files(generator):
    for dirpath, dirnames, filenames in generator:
        for name in list(dirnames):
            if name == "CVS" or name.startswith(".") or name == "__pycache__":
